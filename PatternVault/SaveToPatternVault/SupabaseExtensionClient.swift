...
enum SupabaseExtensionClient {
    ...
    /// Reads shared auth credentials from App Group.
    ...
    /// Reads Supabase config from the extension's shared App Group or falls back to known values.
    private static func config() -> (url: String, anonKey: String)? {
        ...
        // Fallback: read from extension's Info.plist
